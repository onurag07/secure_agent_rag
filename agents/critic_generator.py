...
def generate_response(query: str, retrieved_docs: list[dict]) -> str:
    ...
    try:
        ...
        if retrieved_docs:
            ...
            return (
                f"**Retrieved Answer (LLM API Key Offline / Error):**\n\n{doc_summary}\n\n"
                f"*Note: Please provide a valid `GROQ_API_KEY` in your `.env` file to enable Groq LLM synthesis.*"
            )
        return f"I encountered an issue generating a response. Please check your `GROQ_API_KEY` in `.env`. Details: {e}"